 ...
 *   reminders: Array<{ reminder_id: string, title: string, owner: string[], window_start: string, status?: string }> }} Removed
 */
...
export function removedSection(ctx, removed) {
   ...
   * @param {string} [action]
   */
  const row = (label, detail, restore, done, action = 'Restore') => el('li', { class: 'item with-action' },
  ...
  return el('section', {},
    el('details', {},
      ...
      removed.reminders.length ? el('h3', { class: 'muted small' }, 'Done or cancelled reminders') : '',
      el('ul', { class: 'items' }, removed.reminders.map((r) => row(`${r.owner.join('+')} - ${r.title}`, `${r.status === 'DONE' ? 'done' : 'cancelled'} · from ${niceDate(r.window_start)}`,
        () => ctx.call('reminders.setStatus', { reminder_id: r.reminder_id, status: 'ACTIVE' }), `Reopened "${r.title}".`, 'Reopen'))),
