...
import { icon, toneStyle, shade, schoolLabel } from './parts.js';
 ...
 * @param {string} view  FAMILY, C, R, RT or G
 */
export function monthView(theme, data, monthStart, today, openDay, view) {
  ...
  const cells = data.days.map((day) => {
    ...
    const label = schoolLabel(theme, day.school, view === 'C' || view === 'R');
    return el('button', {
