 ...
 *   participants: string[], related_people: string[], location: string|null, notes: string|null, routine: boolean }} AppItem
...
/**
 * One item as a row: time, coloured label with its icon, and details.
 * @param {Theme} theme
 * @param {AppItem} i
 */
export function itemRow(theme, i) {
...
/**
 * A day as a titled list.
 * @param {Theme} theme
 * @param {string} title
 * @param {AppDay|undefined} day
 * @param {{ onTitle?: () => void }} [options]
 */
export function daySection(theme, title, day, options = {}) {
  ...
  return el('section', { class: 'day-section' },
    ...
    !day || day.items.length === 0
      ...
      : el('ul', { class: 'items' }, day.items.map((i) => itemRow(theme, i))));
}
